...
PCB 艺术画的预览默认采用一个重要的相对色阶：在同一种阻焊颜色中，**有铜且盖阻焊通常更浅、更亮，无铜且盖阻焊通常更深**。因此蓝色或紫色板面可以用“同色不同明度”表现覆铜层次，而不是额外伪造一种油墨颜色。

需要区分：有铜且开窗显示的是沉金或 OSP 表面处理色；无铜且开窗不是金色。这个明暗关系用于预览与设计判断，不是固定 RGB 或实物色卡；油墨、铜面、底材、表面处理、光照和厂家渲染都会影响最终观感。详细规则见 [`references/design.md`](references/design.md)。
